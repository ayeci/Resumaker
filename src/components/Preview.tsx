...
export const Preview: React.FC = () => {
    ...
    // 自動縮尺調整 (初期表示時) - 標準プレビュー時のみ有効
    useLayoutEffect(() => {
        if (previewMode !== 'standard') return;

        const updateScale = () => {
            if (wrapperRef.current) {
                const parent = wrapperRef.current.parentElement;
                if (parent) {
                    const parentWidth = parent.clientWidth - 32; // パディング
                    ...
                    const scaleH = parentHeight / a4HeightPx;
        ...
        const timer = setTimeout(updateScale, 100); // レイアウト確定を待つ
    ...
    const renderStandardPreview = () => {
            ...
            // 志望動機・特技・好きな学科・アピールポイントなど
            {
                id: 'motivation', height: H_MOTIVATION, content: (
                    <Box className={styles.motivationBox}>
                        ...
                        <Typography className={clsx(styles.boxTitle, styles.mt2)}>趣味・特技など</Typography>
        ...
        return pages.map((pageBlocks, pgIdx) => (
            <Paper
                key={`page-${pgIdx}`}
                elevation={3}
    ...
    const renderContent = () => {
        ...
        return <ThemeProvider theme={theme}>{renderStandardPreview()}</ThemeProvider>;
    };
    ...
    return (
        <Box
            ...
            style={{ "--preview-scale": previewMode === 'standard' ? scale : 1 } as React.CSSProperties}
        >
            {renderContent()}
        </Box>
    );
